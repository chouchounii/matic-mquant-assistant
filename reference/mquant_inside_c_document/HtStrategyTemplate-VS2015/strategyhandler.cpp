...
#include "CommonDef.h"
/*
std::string AnsiToUtf8(const char *srcCode)//测试
{
	WCHAR*   strA;
...
//    m_downloadThread.init(context);
	SymbolList symbols;
	//symbols.put("601688.SH");
	//symbols.put("000002.SZ");
	//symbols.put("600000.SH");
	//symbols.put("000001.SZ");
	symbols.put("100303.SZ");

	EtfConstituentList etfInfoList = context->getEtfHandler()->getEtfConstituentList("159919.SZ");
	for (int i = 0; i < etfInfoList.getSize(); i++) {
		EtfConstituentInfo info = etfInfoList.get(i);
//		if (std::string(info.symbol) == "600068.SH")
			MQLogDebug << info;
	}
//	context->getSysApi()->registerFastParamsWnd("FastMsgWnd");

	SecurityDetial detial = context->getSysApi()->getSymbolDetial("000679.SZ");
	MQLogDebug << detial;
	//return;
	/*
	context->getSysApi()->registerBacktestSymbols(symbols);
    ...
    for(int i = 0; i < symbolList.getSize(); i++){
        if(i < perThreadSymbolNum)
            m_thread1Symbols.push_back(symbolList.get(i).str());
        else if(i < 2 * perThreadSymbolNum)
            m_thread2Symbols.push_back(symbolList.get(i).str());
        else if(i < 3 * perThreadSymbolNum)
            m_thread3Symbols.push_back(symbolList.get(i).str());
        else if(i < 4 * perThreadSymbolNum)
            m_thread4Symbols.push_back(symbolList.get(i).str());
        else
            m_thread5Symbols.push_back(symbolList.get(i).str());
    }
	...
	*/
	...
	char szEndTime[64] = { 0 };
	sprintf_s(szStartTime, "%02d:%02d:%02d %d", st.wHour, st.wMinute, st.wSecond + 5, st.wMilliseconds);
	sprintf_s(szEndTime, "%02d:%02d:%02d %d", st.wHour, st.wMinute + 2, st.wSecond, st.wMilliseconds);
//    context->getSysApi()->runTimelyOneShot(szStartTime, "one shot");
    //注册3s执行一次的周期定时信号，3s后开始，2分钟后结束，在timerFuncPeriod中接收定时信号
	
//	context->getSysApi()->runTimelyPeriod(5, szStartTime,szEndTime,"period");
...
//    订阅ETF预估信息
 /*   SymbolList etfSymbols;
	...
	*/
	context->getSysApi()->subscribe(symbols);
	context->getSysApi()->subscribe(symbols, MQuant::SubscribeType::RECORD_ORDER);   //订阅逐笔委托，在handleOrderRecord中接收
	context->getSysApi()->subscribe(symbols, MQuant::SubscribeType::RECORD_TRANSACTION); //订阅逐笔成交，在handleOrderTransaction中接收
...
/*    context->getSysApi()->subscribeCustomMsg(MQUANT_CUSTOM_MSG_TYPE_BEGIN + 1);
    ...
    strcpy_s(orderReq.symbol, "601688.SH");
...
//	context->getSysApi()->startStrategy(".\\testEtf.py");
...
void StrategyHandler::handleData(IHtContext *context, const KLineDataPush *kline)
{
	...
	OrderRequest orderReq;
	memset(&orderReq, 0, sizeof(OrderRequest));
	strcpy_s(orderReq.symbol, kline->symbol);
	orderReq.amount = 10000;
	orderReq.side = OrderSide::BUY;
	orderReq.price = kline->high;
	strcpy_s(orderReq.price_type, PriceType::LIMIT.str());
	orderReq.entrust_type = EntrustType::ENTRUST;       //可不赋值，memset初始化时会默认设置为普通委托
//	MQLogDebug << UTF8STR("策略正式进入运行阶段，开始报单:") << orderReq.symbol << "," << orderReq.amount << "," << orderReq.side;
	context->getSysApi()->order(MQuant::AccountType::STOCK, orderReq);
...
void StrategyHandler::handleOrderRecord(IHtContext *context, const RecordOrder *recordOrder)
{
	...
	//RecordOrder591 *p = (RecordOrder591*)recordOrder;
	//MQLogDebug << UTF8STR("接收到逐笔委托591：") << *p;
}

void StrategyHandler::handleOrderTransaction(IHtContext *context, const RecordTransaction *recordTrans)
{
	MQLogDebug << UTF8STR("接收到逐笔成交：") << (__int64)GetCurrentThreadId() << *recordTrans;
	//RecordTransaction591 *p = (RecordTransaction591*)recordTrans;
	//MQLogDebug << UTF8STR("接收到逐笔成交591：") << *p;
    ...
    if(!m_batchOrderFlag){
        ...
        for(int i = 0;  i < etfConstituents.getSize(); i++){
            if(marginCashStocks.index(etfConstituents.get(i).symbol) < 0)
                continue;
            strcpy_s(batchOrderReq.req_list[j].symbol, etfConstituents.get(i).symbol);
            batchOrderReq.req_list[j].amount = etfConstituents.get(i).sample_size - etfConstituents.get(i).sample_size % 100;
            strcpy_s(batchOrderReq.req_list[j].price_type, PriceType::LIMIT.str());
...
{
//	EtfConstituentList etfConstituents = context->getEtfHandler()->getEtfConstituentList("512500.SH");
	IndexComponentList componentList = context->getSysApi()->getIndexComponents("000016.SH");
	//查询融资标的
	//SymbolList marginCashStocks = context->getMarginHandler()->getMarginCashStocks();
	OrderQueryReq orderQryReq;
	memset(&orderQryReq, 0, sizeof(OrderQueryReq));
	...
	MQLogDebug << UTF8STR("开始买入上证50成分券");
	BatchOrderReq batchOrderReq;
	memset(&batchOrderReq, 0, sizeof(BatchOrderReq));
	batchOrderReq.batch_no = 1000;
	batchOrderReq.count = componentList.getSize();
	batchOrderReq.req_list = new OrderRequest[batchOrderReq.count];
	memset(batchOrderReq.req_list, 0, sizeof(OrderRequest) * batchOrderReq.count);
	int j = 0;
	for (int i = 0; i < componentList.getSize(); i++) {

		strcpy_s(batchOrderReq.req_list[j].symbol, componentList.get(i).symbol);
		batchOrderReq.req_list[j].amount = (int)(componentList.get(i).weight * 1000) - (int)(componentList.get(i).weight * 1000) % 100;
		strcpy_s(batchOrderReq.req_list[j].price_type, PriceType::LIMIT.str());
		batchOrderReq.req_list[j].price = 5.0;
		batchOrderReq.req_list[j].entrust_type = EntrustType::ENTRUST;
		batchOrderReq.req_list[j].side = OrderSide::BUY;
		j++;
	}
	batchOrderReq.count = j;
	MQLogDebug << UTF8STR("订单组装完成");
	context->getSysApi()->batchOrders(AccountType::STOCK, batchOrderReq);
	m_batchOrderFlag = true;
	delete[]batchOrderReq.req_list;
	MQLogDebug << UTF8STR("结束买入上证50成分券");
	/*
    MQLogDebug << UTF8STR("接收到单次定时信号:") << time << customField;
    //查询K线并打印
    KLineQueryReq req;
    memset(&req, 0, sizeof(KLineQueryReq));
    strcpy_s(req.symbol, "601688.SH");
    strcpy_s(req.kline_type , KLineDataType::KLINEDATA_1M.str());
    ...
    strcpy_s(reqFromInitDate.symbol, "601688.SH");
    strcpy_s(reqFromInitDate.kline_type , KLineDataType::KLINEDATA_1D.str());
    reqFromInitDate.date_type = DateType::TRADE;
    reqFromInitDate.days = -30;
	SYSTEMTIME st;
	GetLocalTime(&st);
    reqFromInitDate.init_date = st.wYear*10000 + st.wMonth * 100 + st.wDay;
...
void StrategyHandler::onRspUserParams(IHtContext * context, const char * jsonParams, const char * wndTitle)
{
	MQLogDebug << UTF8STR("请求用户参数响应:") << jsonParams << UTF8STR(",窗口标题：") << wndTitle;
	
}

void StrategyHandler::onRequestUserParamsTemplate(IHtContext * context, const char * params)
{
	MQLogDebug << UTF8STR("用户获取参数模板请求:") << params;
	char szParamTemplate[1024] = { 0 };
	sprintf_s(szParamTemplate, UTF8STR("{\"left_layout\":{\"name\":\"操作表格\",\"ui_type\":\"table\",\"columns\":[{\"name\":\"\",\"ui_type\":\"check_box\"},"
	"{\"name\":\"证券代码\", \"ui_type\" : \"code_edit\"}, { \"name\":\"持仓数量\",\"ui_type\" : \"line_edit\",\"editable\" : \"0\" },"
	"{ \"name\":\"可用数量\",\"ui_type\" : \"line_edit\",\"editable\" : \"0\" },{ \"name\":\"持仓均价\",\"ui_type\" : \"line_edit\",\"editable\" : \"0\" }, "
	"{ \"name\":\"止损价\",\"ui_type\" : \"line_edit\" }],\"rows\":[[0, \"601688.SH\", 5400, 1800, 29.15, 29.15], [0, \"000001.SZ\", 3800, 3000, 18.15, 18.15]]}}"));
	HtString strParams = context->getSysApi()->getUserParams(szParamTemplate, "TestWnd", TRUE);

	MQLogDebug << UTF8STR("请求到的参数为：") << strParams;
}

void StrategyHandler::onFastParams(IHtContext * context, const char * params)
{
	MQLogDebug << UTF8STR("快速窗口参数:") << params;
}
#include <iostream>
void StrategyHandler::onFundUpdate(IHtContext * context, const FundInfo * fundInfo)
{
	MQLogDebug << UTF8STR("资金更新:") << fundInfo->fund_account << "," << fundInfo->available_cash << "," << fundInfo->total_value;
}

void StrategyHandler::onPositionUpdate(IHtContext * context, const Position * posInfo)
{
	MQLogDebug << UTF8STR("持仓更新：:") << posInfo->fund_account << "," << posInfo->closeable_amount << "," << posInfo->symbol << "," << posInfo->total_amount;
}
...
void StrategyHandler::testMargin(IHtContext *context)
{
    ...
    strcpy_s(req.price_type , PriceType::BEST5_OR_CANCEL.str());
...
//    HtLog::setLogLevel(LOG_INFO);
    ...
    //注册自定义日志，存储订单数据
	std::string folderPath = "D:\\MQuantData";
